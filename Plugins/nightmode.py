...
def nightmode_callback_handler(c, m):
    ...
    if data.startswith("nightmode_toggle:"):
        ...
        if is_enabled:
            # تعطيل اعدادات المضاد
            r.delete(f"{chat_id}:nightmode:enabled:{Dev_Neptune}")
            status_text = "×"
            status_word = "غير مفعل"
            message_text = "تم تعطيل اعدادات المضاد بنجاح"
        else:
            # تفعيل اعدادات المضاد
            r.set(f"{chat_id}:nightmode:enabled:{Dev_Neptune}", "1")
            status_text = "√"
            status_word = "مفعل"
            message_text = "تم تفعيل اعدادات المضاد بنجاح"
    ...
    # اعدادات المضاد
        ...
        links_status = "√" if r.get(f"{chat_id}:nightmode:links:{Dev_Neptune}") else "×"
        forward_status = "√" if r.get(f"{chat_id}:nightmode:forward:{Dev_Neptune}") else "×"
        contacts_status = "√" if r.get(f"{chat_id}:nightmode:contacts:{Dev_Neptune}") else "×"
        photos_status = "√" if r.get(f"{chat_id}:nightmode:photos:{Dev_Neptune}") else "×"
        videos_status = "√" if r.get(f"{chat_id}:nightmode:videos:{Dev_Neptune}") else "×"
        animations_status = "√" if r.get(f"{chat_id}:nightmode:animations:{Dev_Neptune}") else "×"
        stickers_status = "√" if r.get(f"{chat_id}:nightmode:stickers:{Dev_Neptune}") else "×"
        files_status = "√" if r.get(f"{chat_id}:nightmode:files:{Dev_Neptune}") else "×"
    ...
    # تبديل حالة أنواع المحتوى المختلفة
        ...
        return m.edit_message_text(
            f"{status} {content_name} في اعدادات المضاد",
            reply_markup=keyboard
        )
...
def nightmode_monitor(c, m):
    try:
        ...
        # التحقق من تفعيل اعدادات المضاد
        ...
        print(f"خطأ في مراقب اعدادات المضاد: {e}")
