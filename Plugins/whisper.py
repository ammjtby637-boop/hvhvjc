...
async def english_whisper(app,iquery):
    ...
    url = 'https://l.top4top.io/p_3434znneu1.jpg'
    id = get_id()
...
async def arabic_whisper(app,iquery):
    ...
    if user == "all":
      ...
      username = get.first_name
      user_name = get.username
      text = f"**هاي الهمسة للحلو ( @{user_name} ) هو اللي يكدر يشوفها 🕵️**"
    url = 'https://g.top4top.io/p_350002ba30.jpg'
    id = get_id()
    r.set(f'{id}', f'id={user_id}+{user}&whisper={query}',ex=86400)
    reply_markup = InlineKeyboardMarkup(
      [[
...
async def get_whisper(app,query):
  if query.data.endswith('+ar'):
    ...
    if r.get(id):
      ...
      if not 'all' in id and not str(query.from_user.id) in id and not query.from_user.id == 651286114:
        return await query.answer('~ الهمسة مو الك يا حبيبي',show_alert=True, cache_time=600)
    reply_markup = InlineKeyboardMarkup(
      [
      [
    ...
    if "all" in id:
      ...
      if str(query.from_user.id) in id.split('+')[1]:
         ...
         try:
           ...
           pass
      if query.from_user.id == 651286114 or query.from_user.id ==651286114:
         return await query.answer(q[:200], show_alert=True, cache_time=600)
  else:
    id = query.data.split("whisper")[0]
    if r.get(id):
      ...
      await query.answer(
        ...
        results=[           
            InlineQueryResultArticle(
                ...
                thumb_url='https://t.me/siznr/54',
                thumb_width=128, thumb_height=128,
                description='@0bot Hello @usrname',
      ...
      await query.answer(
        ...
        results=[           
            InlineQueryResultArticle(
                ...
                thumb_url='https://t.me/siznr/54',
                thumb_width=128, thumb_height=128,
                description='@1bot Hello @usrname',
                url='https://t.me/iJUCK',
                reply_markup=InlineKeyboardMarkup (
                [
