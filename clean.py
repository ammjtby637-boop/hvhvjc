...
Neptune = token.split(':')[0]


@DevNeptune.on_message(filters.group & filters.media, group=1)
async def add_messages(c: Client, m: Message):
    ...
    if (m.media) and not m.audio and not m.voice and not m.game:
        if await db.hget(Neptune+str(m.chat.id), "ena-clean"):
            secs = int(await db.hget(Neptune+chat_id, "clean-secs") or "60")
            time_now = datetime.now()
            data = {"id":m.id, "time":time_now + timedelta(seconds=secs)}
            chats_db[m.chat.id].append(data)
            
    
    if m.media_group_id and await db.hget(Neptune+str(m.chat.id), "ena-clean"):
        secs = int(await db.hget(Neptune+chat_id, "clean-secs") or "60")
        time_now = datetime.now()
    ...
    while not await asyncio.sleep(1.7):
        try:
            for chat_id in chats_db:
                ...
                try:
                    await DevNeptune.delete_messages(chat_id, msgs_ids)
...
async def main():
    await DevNeptune.start()
    print(DevNeptune.me.username)
