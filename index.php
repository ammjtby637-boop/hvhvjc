...
mkdir('data/txt');
mkdir('data/api');
$API_KEY= '8516176029:AAG7xHewCg2RkZ-PH8-nnBszVKh8nGV4QFM';
...
$numbots = (is_array($ORDERALL)) ? count($ORDERALL) : 0;
 #العدد الكلي لكمية الأرقام المستخرجة#
...
$BUYSPRIC = (file_exists("EMILS/$EM/price.json")) ? json_decode(file_get_contents("EMILS/$EM/price.json"),true) : [];
#_________________
...
$readymy = isset($BUYSNUM['ready_my']) ? $BUYSNUM['ready_my'] : 0; #عدد الأرقام الجاهزة#
$orderall = (is_array($ORDERALL)) ? count($ORDERALL)+1 : 1; #عدد مشتريات الاعضاء#
$idnums = (is_array($ORDERALL)) ? count($ORDERALL)+999999999 : 999999999; #عدد مشتريات الاعضاء#
$cardmy = (is_array($BUYSCARD)) ? count($BUYSCARD) : 0; #عدد الكروت المشترى#
$sendmy = (is_array($BUYSSEND)) ? count($BUYSSEND) : 0; #عدد عمليات تحويل الروبل#
$pricmy = (is_array($BUYSPRIC)) ? count($BUYSPRIC) : 0; #عدد عمليات شحن الحساب#
$buymy = isset($BUYSNUM['number_my']) ? $BUYSNUM['number_my'] : 0; #عدد الأرقام المكتملة#
$rubles = (file_exists("EMILS/$EM/rubles.txt")) ? file_get_contents("EMILS/$EM/rubles.txt") : 0; #الرصيد اللكلي#
$Balance = (file_exists("EMILS/$EM/points.txt")) ? file_get_contents("EMILS/$EM/points.txt") : 0; #رصيد العضو#
$consumers = (is_numeric($rubles) && is_numeric($Balance)) ? ($rubles - $Balance) : 0; #عدد الروبل المستهلك#
#_________________
#_________________
if($numbuy==null){
$numbuy=0;
...
$chall = "fv_mv";
$useradmin = "fvanv";
$sim =-1002915550218; #القناة الرسمية
$PAY =-1002915550218; #الارقام المكتملة
$ess =-1002915550218; #عمليات شراء الارقام
$eer =-1002915550218; #اشعارات الايميلات وتحويل الروبل
if($chat_id==8065884629){
$sudo=$chat_id;
}elseif($chat_id==8065884629){
$sudo=$chat_id;
}else{
$sudo==8065884629;
}
